...
async def get_if_name_table(device: DriverBase, community) -> dict:
    ...
    recs = await walk_table(
        device, oid=ODI_IF_NAME, community=community, factory=_table_by_index
    )
    return dict(recs)
...
async def get_if_alias_table(device: DriverBase, community) -> dict:
    ...
    """
    recs = await walk_table(
        device, oid=OID_IF_DESC, community=community, factory=_table_by_index
    )
    return dict(recs)
